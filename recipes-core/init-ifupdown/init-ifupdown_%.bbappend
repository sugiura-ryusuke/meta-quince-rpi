...
do_install:append() {
	...
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi3', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi3-64', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi4', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi4-64', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi5', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi0-2w', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo ${D}${sysconfdir}/network/interfaces
	elif ${@bb.utils.contains('MACHINE', 'raspberrypi0-2w-64', 'true', 'false', d)}; then
		install -m 0644 ${WORKDIR}/interfaces_lo ${D}${sysconfdir}/network/interfaces
	else
		install -m 0644 ${WORKDIR}/interfaces_lo_eth0 ${D}${sysconfdir}/network/interfaces
	fi
