...
# example
#   QUINCE_TARGET_DEPLOY_DIR : target-quince-image-minimal-raspberrypi3-64
...
cp ${DEPLOY_DIR_IMAGE}/${IMAGE_LINK_NAME}.wic.bz2 ${QUINCE_TARGET_DEPLOY_DIR}
...
if [ -e ${DEPLOY_DIR_IMAGE}/bootfiles/cmdline.txt ]; then
	cp ${DEPLOY_DIR_IMAGE}/bootfiles/cmdline.txt ${QUINCE_TARGET_DEPLOY_DIR}/boot
fi
if [ -e ${DEPLOY_DIR_IMAGE}/bootfiles/config.txt ]; then
	cp ${DEPLOY_DIR_IMAGE}/bootfiles/config.txt ${QUINCE_TARGET_DEPLOY_DIR}/boot
fi
if [ -e ${DEPLOY_DIR_IMAGE}/Image ]; then
	cp ${DEPLOY_DIR_IMAGE}/Image ${QUINCE_TARGET_DEPLOY_DIR}/boot
fi
if [ -e ${DEPLOY_DIR_IMAGE}/u-boot.bin ]; then
	cp ${DEPLOY_DIR_IMAGE}/u-boot.bin ${QUINCE_TARGET_DEPLOY_DIR}/boot
fi


mkdir -p ${QUINCE_TARGET_DEPLOY_DIR}/boot/overlays
DTBO_FILES=`find ${DEPLOY_DIR_IMAGE}/*.dtbo -type f`
cp ${DTBO_FILES} ${QUINCE_TARGET_DEPLOY_DIR}/boot/overlays
